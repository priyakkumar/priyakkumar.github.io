...
$(document).ready(function() {
	...
	for (i=0; i<24; i++)	{
		...
		if (i==12) {
			$('#board').append("<div data-value='1' class='selected freesquare' id='sqfree'><div class='text'><img src='logo.png'</div></div>");
			$('#board').append("<div data-value='0' class='square' id='sq12'><div class='text'><br/><p>"+JSONBingo.squares[i].square+"</p></div></div>");
		} else {
			$('#board').append("<div data-value='0' class='square' id='sq"+i+"'><div class='text'><br/><p>"+JSONBingo.squares[i].square+"</p></div></div>");
		}
  }
	...
	$('div.square').tappable(function () {
      ...
      if ($(this).data('value') == 1) {
		...
		if (row1 == 5 || row2 == 5 || row3 == 5 || row4 == 5 || row5 == 5 || col1 == 5 || col2 == 5 || col3 == 5  || col4 == 5  || col5 == 5 || diag1 == 5 || diag2 == 5) {
			$('#wintext').fadeIn(1000);
			$('#wintext').html(winText);
			$('#wintext').append('</br><a href="javascript:location.reload(true)">Play again.</a>')
			// $('#header').addClass("win");
			// $( "#intro" ).hide();
